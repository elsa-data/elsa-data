...
Within the system permissions, there are the following permissions that stick to every account.
...
- `isAllowedCreateRelease` - This permission allows the user to create a new release from existing dataset records. Making new releases will become an administrator role in the context of release.
- `isAllowedRefreshDatasetIndex` - This permission allows the user to update the current dataset by fetching the latest data from its respective data-storage. This includes fetching data-egress records if configured.
- `isAllowedChangeUserPermission` - This is a special permission that allows changing user permissions. This permission itself is mutable only through the system configuration, and changing this requires a system reboot.
...
- `Administrator` - This role is given to the data custodian that created the release. (This user will have the `isAllowedCreateRelease` permission from the user permissions above.)
- `Manager` - This role is given to the Person In Charge (PIC) / research lead for the release.
- `Member` - This role is given by the `Manager`/`Administrator` to access the data in the release. The use case for this is a person from the PIC research group need to access the given data, but instead of using the `Manager` credentials to retrieve it, the manager could delegate this (`Member`) role to that person.

Permissions bound on each role are as follows.

|                                                                                                                                        | `Administrator`    | `Manager`          | `Member`           |
| -------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------ | ------------------ |
| Modify cases/patients to be included in the release                                                                                    | :white_check_mark: |                    |                    |
| Modify files to be included in the release                                                                                             | :white_check_mark: |                    |                    |
| Modify what type of method allowed for the data access<br>(e.g. only allow HTSGET endpoint)                                            | :white_check_mark: |                    |                    |
| Enable/suspend data sharing in a release                                                                                               | :white_check_mark: |                    |                    |
| Add/remove `Member` from the release                                                                                                   | :white_check_mark: | :white_check_mark: |                    |
| Promote/Demote `Manager` from/to a `Member` role                                                                                       | :white_check_mark: | :white_check_mark: |                    |
...
| Accessed the data-content available in the release.                                                                                    |                    | :white_check_mark: | :white_check_mark: |

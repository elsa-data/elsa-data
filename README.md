...
| `isAllowedCreateRelease`            | Allow user to create a new release from existing dataset records. Making new releases will become an administrator role in the context of release.                                           |
...
| `Member`        | Given by the `Manager`/`Administrator` to access the data in the release. The use case for this is a person from the PIC research group need to access the given data, but instead of using the `Manager` credentials to retrieve it, the manager could delegate this (`Member`) role to that person. |

...
export class ReleaseActivationPermissionError extends Base7807Error {
  constructor(releaseId: string) {
    super(
      ...
      400,
      `The release with id '${releaseId}' cannot have its activation state changed by this user`
    );
  }
}
...
export class ReleaseActivationNoEditingAllowedError extends Base7807Error {
  constructor(releaseId: string) {
    super(
      ...
      400,
      `The release with id '${releaseId}' is activated and hence cannot be edited`
    );
  }
}
...
export class ReleaseActivationStateError extends Base7807Error {
  constructor(releaseId: string) {
    super(
      ...
      400,
      `The release with id '${releaseId}' is activated and hence cannot be activated again`
    );
  }
}
...
export class ReleaseDeactivationStateError extends Base7807Error {
  constructor(releaseId: string) {
    super(
      ...
      400,
      `The release with id '${releaseId}' is deactivate and hence cannot be deactivated again`
    );
  }
}

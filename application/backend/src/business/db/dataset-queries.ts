...
/**
 * Returns an edgedb query for counting datasets.
 */
export function datasetAllCountQuery() {
  return e.count(e.dataset.Dataset);
}

/**
 * Returns an edgedb query for all our summary dataset info + counts/calcs. It *does not*
 ...
 */
export function datasetAllSummaryQuery(limit: number, offset: number) {
  return e.select(e.dataset.Dataset, (ds) => ({
    ...
    summaryArtifactBytes: e.sum(
      e.set(
        ...
        e.sum(
          ds.cases.patients.specimens.artifacts.is(e.lab.ArtifactCram).cramFile
            .size
        ),
        e.sum(
          ds.cases.patients.specimens.artifacts.is(e.lab.ArtifactCram).craiFile
            .size
        ),
        e.sum(
